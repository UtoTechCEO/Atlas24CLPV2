(function() {
    ...
    const locale_de = {
        ...
        months: ['Januar', 'Februar', 'März', 'April', 'Mai', 'Juni', 'Juli', 'Aug', 'Sep', 'Okt', 'Nov', 'Dez'],
    ...
    const validation_reg_exp = {
        ...
        phone: /^\s*(?:\+41(?!\s*(?:\(0\)|0))|0041(?:\s*\(0\))?|0)\s*7[5-9]\s*\d{3}\s*\d{2}\s*\d{2}\s*$/,
...
>>>>>>> 0b95f7a7ba358abb36b2b0ad60a9eee6d8d2445a
    ...
    function init_pagination() {
        btn_fwd.addEventListener('click', () => {
            if ((view_index + 1) <= end_index && validate_current_section()) {
                view_index++;
                update_views();
            }
        });
...
>>>>>>> 0b95f7a7ba358abb36b2b0ad60a9eee6d8d2445a
    ...
    class atlui_form_field {
        ...
        focus_out() { setTimeout(this.validate.bind(this), 200); }
    }
    ...
    function add_element_to_validation_map(key, elem) {
        ...
        if(validation_method_name) {
            ...
            console.warn(`[ATLAS_24] <input/textarea> missing ${ATTRIBUTE_VALIDATION_METHOD}: ${elem.name}`);
            return;
        }
    ...
    function remove_element_from_validation_map(key) {
        if(atlui_validation_map.has(key))
            atlui_validation_map.delete(key);
    }

    function initialize_element_validation() {
        document.querySelectorAll('input[class*="atlui_block"], textarea[class*="atlui_block"]').forEach((elem) => {
    ...
    const get_rgb_for_progress = (percent) => { 
        switch(percent) {
            case 10: case 15: case 20: case 25: case 30: case 35: return 'rgb(234, 0, 0)';
            case 40: case 45: case 50: case 55: case 60: case 65: return 'rgb(234, 165, 0)';
            case 70: case 75: case 80: case 85: case 90: case 90: return 'rgb(234, 204, 0)';
            case 100: return 'rgb(90, 165, 0)';
        }
        return;
    }; 
    ...
    function validate_current_section() {
        ...
        const current_section_elements = document.querySelectorAll(
            `[data-step-index="${view_index}"] input:not(.ignore), [data-step-index="${view_index}"] textarea:not(.ignore)`
        );
            ...
            if(elem.type !== 'radio' && elem.type !== 'checkbox' ) {
                switch(elem.name) {
                    ...
                    case 'MoveToAddrStreet':
                        elem_is_valid = validation_methods.contains_street_num(elem.value); break;
                    case 'MoveFromAddrZip':
                    case 'MoveToAddrZip':
                        elem_is_valid = validation_methods.swiss_zip_code(elem.value); break;
                    case 'Email':
                        elem_is_valid = validation_methods.email(elem.value); break;
                    case 'Phone':
